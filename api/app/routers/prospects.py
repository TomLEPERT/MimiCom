...
from fastapi import APIRouter, File, UploadFile, Query, status, HTTPException
...
from ..models.import_report import ImportReport, ImportRowError, ImportDuplicate
from ..utils.enum import ForceMode, as_set
...
async def import_prospects_csv(
    ...
    force_mode: ForceMode = Query(default=ForceMode.none),
    force_rows: Optional[List[int]] = Query(default=None),
):
    """
    Import CSV de prospects.

    - dry_run=true  -> preview uniquement
    - dry_run=false -> insertion réelle

    Forçage (doublons DB) via force_mode :
    - none       : aucun doublon DB autorisé
    - all        : autorise doublons email + téléphone
    - email      : autorise doublons email uniquement
    - telephone  : autorise doublons téléphone uniquement
    - rows       : autorise doublons DB seulement pour les lignes listées dans force_rows
    """

    col = get_prospects_collection()

    forced_rows_set = as_set(force_rows)
    ...
    # doublons au sein du CSV
    seen_email: Dict[str, int] = {}
    seen_tel: Dict[str, int] = {}

    # infos de normalisation par ligne
    email_norms: Dict[int, str] = {}
    tel_norms: Dict[int, str] = {}

    # lignes valides + data
    valid_docs: Dict[int, Dict[str, Any]] = {}

    # ------------------------------------------------------------
    # Validation + doublons CSV
    # ------------------------------------------------------------
    for row_i, row in enumerate(reader, start=1):
        total += 1

        cleaned = {
            (k.strip() if isinstance(k, str) else k): (v.strip() if isinstance(v, str) and v.strip() != "" else None)
        ...
        try:
            ...
            for e in ve.errors():
                errors.append(
                    ImportRowError(
                        row=row_i,
                        field=".".join(map(str, e.get("loc", []))),
                        message=e.get("msg", "Erreur validation"),
                        raw=cleaned,
                    )
                )
            continue

        email_norm = normalize_email_for_db(cleaned.get("email")) if cleaned.get("email") else None
        tel_norm = normalize_phone_for_db(cleaned.get("telephone")) if cleaned.get("telephone") else None
            ...
            if email_norm in seen_email:
                duplicates.append(
                    ImportDuplicate(
                        ...
                        value=email_norm,
                        conflict="csv",
                        other_row=seen_email[email_norm],
                    )
                )
                continue
            seen_email[email_norm] = row_i
            email_norms[row_i] = email_norm
            ...
            if tel_norm in seen_tel:
                duplicates.append(
                    ImportDuplicate(
                        ...
                        value=tel_norm,
                        conflict="csv",
                        other_row=seen_tel[tel_norm],
                    )
                )
                continue
            seen_tel[tel_norm] = row_i
            tel_norms[row_i] = tel_norm

        valid_docs[row_i] = cleaned

    # ------------------------------------------------------------
    # Doublons DB (on détecte, puis on décide par ligne si bloqué)
    # ------------------------------------------------------------
    existing_emails: Dict[str, str] = {}  # email_norm -> prospect_id
    existing_tels: Dict[str, str] = {}    # tel_norm -> prospect_id

    if seen_email:
        async for doc in col.find(
            {"email_norm": {"$in": list(seen_email.keys())}, "allow_duplicate": {"$ne": True}},
            {"email_norm": 1, "prospect_id": 1},
        ):
            existing_emails[doc["email_norm"]] = doc["prospect_id"]

    if seen_tel:
        async for doc in col.find(
            {"telephone_norm": {"$in": list(seen_tel.keys())}, "allow_duplicate": {"$ne": True}},
            {"telephone_norm": 1, "prospect_id": 1},
        ):
            existing_tels[doc["telephone_norm"]] = doc["prospect_id"]

    # rows_blocked : lignes qu'on n'insérera pas si dry_run=false
    rows_blocked: Set[int] = set()

    def can_force_row(row_i: int) -> bool:
        """true si force_mode==rows et la ligne est dans force_rows"""
        return force_mode == ForceMode.rows and row_i in forced_rows_set

    def allow_email_duplicate(row_i: int) -> bool:
        """email duplicable ?"""
        return force_mode in (ForceMode.all, ForceMode.email) or can_force_row(row_i)

    def allow_tel_duplicate(row_i: int) -> bool:
        """telephone duplicable ?"""
        return force_mode in (ForceMode.all, ForceMode.telephone) or can_force_row(row_i)

    # Email DB conflicts
    for row_i, en in email_norms.items():
        if en in existing_emails:
            duplicates.append(
                ImportDuplicate(
                    row=row_i,
                    field="email",
                    value=en,
                    conflict="db",
                    existing_prospect_id=existing_emails[en],
                )
            )
            if not allow_email_duplicate(row_i):
                rows_blocked.add(row_i)

    # Tel DB conflicts
    for row_i, tn in tel_norms.items():
        if tn in existing_tels:
            duplicates.append(
                ImportDuplicate(
                    row=row_i,
                    field="telephone",
                    value=tn,
                    conflict="db",
                    existing_prospect_id=existing_tels[tn],
                )
            )
            if not allow_tel_duplicate(row_i):
                rows_blocked.add(row_i)

    # ------------------------------------------------------------
    # INSERTION (si dry_run=false)
    # ------------------------------------------------------------
    inserted = 0
    skipped = 0
    ...
    if not dry_run:
        ...
        for row_i, data in valid_docs.items():
            # si doublon DB bloquant -> on skip
            if row_i in rows_blocked:
                skipped += 1
                continue

            email_norm = normalize_email_for_db(data.get("email")) if data.get("email") else None
            tel_norm = normalize_phone_for_db(data.get("telephone")) if data.get("telephone") else None

            # On prépare le doc final à insérer
            doc = data.copy()
            doc["prospect_id"] = str(uuid4())

            # allow_duplicate = True si on force au moins un doublon DB sur cette ligne
            forced_email = False
            forced_tel = False
            ...
            if email_norm:
                ...
                # si doublon DB email et autorisé -> bypass unique key
                if email_norm in existing_emails and allow_email_duplicate(row_i):
                    forced_email = True
                    doc["email_unique_key"] = f"FORCED:{uuid4()}"
                else:
                    doc["email_unique_key"] = email_norm

            # Telephone : set norm + key
            if tel_norm:
                doc["telephone_norm"] = tel_norm

                if tel_norm in existing_tels and allow_tel_duplicate(row_i):
                    forced_tel = True
                    doc["telephone_unique_key"] = f"FORCED:{uuid4()}"
                else:
                    doc["telephone_unique_key"] = tel_norm

            # allow_duplicate reflète le fait qu'on a forcé quelque chose
            doc["allow_duplicate"] = bool(forced_email or forced_tel)
    ...
    # REPORT FINAL
    # ------------------------------------------------------------
    # invalid_rows : nombre de lignes distinctes ayant au moins une erreur
    invalid_rows = len({e.row for e in errors})

    # skipped = lignes valides mais bloquées (doublons DB non forcés)
    return ImportReport(
        total_rows=total,
